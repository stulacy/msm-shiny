...
server <- function(input, output) {
    ...
    output$newdata <- renderUI({
        ...
        # Find all covar_names that are in terms
        all_covar_names <- names(all_df()[!grepl(".*\\.time", names(all_df())) & 
                                      !grepl(".*\\.status", names(all_df())) & 
                                      names(all_df()) != "id"])
        
        # Find which of these are in the models
        used_covars <- all_covar_names[sapply(all_covar_names, function(attr) {
            any(grepl(paste0(attr, "[a-zA-Z]*\\.[0-9]+"), names(coef(mod))))
        })]
    ...
    get_newdata <- function(mod) {
        ...
        newd <- data.frame(trans=seq(num_trans), strata=mod$strata)
    ...
    transprobs <- eventReactive(input$updatepred, {
        ...
        withProgress(message="Calculating...", {
                     predict_transition(mod, get_newdata(mod), times) %>%
                        filter(source == starting_state())
        })
    })
